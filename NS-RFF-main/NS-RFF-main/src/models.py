...
class BaseCLF2(nn.Module):
    ...
    def features(self, input):
        ...
        out = self.main_module(input_img).view(N, -1) # view拉直
        return out # 返回了N个样本的结果？
...
class Synchronization(nn.Module):
    def __init__(self, d=4):
        super().__init__()
        self.freq_estimation = BaseCLF2(2, out_dim=1, d=d) # 频偏估计网络
        self.phase_estimation = BaseCLF2(2, out_dim=1, d=d) # 相位估计网络
...
class NS_CLF_Softmax(nn.Module):
    def __init__(self, in_channels=3, out_channels=10, d1=8, d2=24, z_dim=512):
        ...
        self.main_module = BaseCLF2(2, out_dim=z_dim, d=d2) # 关键 zdim d2 = 24， 输出的dim是512
        self.output = nn.Linear(z_dim, out_channels)
...
class NS_CLF_L2Softmax(nn.Module):
    def __init__(self, in_channels=3, out_channels=10, d1=8, d2=24, z_dim=512):
        ...
        # Output_dim = 1
        self.synchronization = Synchronization(d=d1) # 
        self.main_module = BaseCLF2(2, out_dim=z_dim, d=d2) # classifier主干网络
        self.output = ArcMarginProduct(z_dim, out_channels, s=10, m=0.0) # 分类头
