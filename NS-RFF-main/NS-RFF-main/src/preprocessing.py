...
def freq_compensation(
    ...
    segment_length = len(segment) # 区别在有N提取batch的个数
...
def freq_offset_estimation( # 传统方法
        segment, 
...
def phase_compensation( # 对单条信号做相位补偿
        segment,
        phase
    ):
    segment_length = len(segment)
    exp_phase = np.exp(-1j*phase) # 乘以一个相位旋转因子 
    exp_phase = tc.complex2tensor(exp_phase).view(-1, 2).repeat(segment_length, 1) # 先转换成复数的( , 2),再在长度上repeat
    segment = tc.prod(segment, exp_phase) # 点乘
    return segment
